<?php
/**
 * Template para card de evento com controle de permissões organizacionais
 * 
...
// Variáveis $evento e $user_id devem estar disponíveis no contexto
if (!isset($evento) || !isset($user_id)) {
    return;
}
...
<div class="sevo-evento-card" data-evento-id="<?php echo esc_attr($evento->id); ?>">
    ...
    <div class="sevo-card-actions">
        <button type="button" class="sevo-btn sevo-btn-secondary" 
                onclick="SevoEventosDashboardWithPermissions.openEventModal(<?php echo esc_attr($evento->id); ?>)">
        ...
        <?php if ($can_manage_events): ?>
            <button type="button" class="sevo-btn sevo-btn-primary" 
                    onclick="SevoEventosDashboardWithPermissions.editEvent(<?php echo esc_attr($evento->id); ?>)">
