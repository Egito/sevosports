<?php
/**
 * Model para Inscrições com controle de permissões organizacionais
 * 
...
class Sevo_Inscricao_Model_With_Permissions extends Sevo_Base_Model {
    ...
    /**
     * Busca inscrições filtradas por permissões do usuário
     */
    public function get_for_user($user_id = null, $page = 1, $per_page = 10, $filters = []) {
        if ($user_id === null) {
        ...
        if (user_can($user_id, 'manage_options')) {
            return $this->get_paginated($page, $per_page, $filters);
        }
    ...
    /**
     * Busca inscrições de um usuário específico filtradas por permissões do usuário logado
     */
    public function get_user_inscricoes_for_user($target_user_id, $logged_user_id = null) {
        ...
        if (user_can($logged_user_id, 'manage_options')) {
            return $this->get_user_inscricoes_with_relations($target_user_id);
        }
        
        // Verificar se o usuário logado pode ver as inscrições do usuário alvo
        // (mesma organização)
        if (!class_exists('Sevo_Usuario_Organizacao_Model')) {
            require_once SEVO_EVENTOS_PLUGIN_DIR . 'includes/models/Usuario_Organizacao_Model.php';
        }
        ...
        // Verificar se ambos os usuários estão na mesma organização
        $logged_user_orgs = $user_org_model->get_user_organizations($logged_user_id);
        $target_user_orgs = $user_org_model->get_user_organizations($target_user_id);
        
        $logged_org_ids = wp_list_pluck($logged_user_orgs, 'organizacao_id');
        $target_org_ids = wp_list_pluck($target_user_orgs, 'organizacao_id');
        
        // Se não há interseção de organizações, não mostrar inscrições
        if (empty(array_intersect($logged_org_ids, $target_org_ids))) {
            return [];
        }
    ...
    public function get_paginated($page = 1, $per_page = 10, $filters = []) {
        ...
        if (!empty($filters['organizacao_ids']) && is_array($filters['organizacao_ids'])) {
